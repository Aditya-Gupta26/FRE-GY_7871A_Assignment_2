...
Kevin Warsh became Fed Chair in May 2026, succeeding Jerome Powell. This project asks three questions: **has the tone of Fed communication changed** since the handover, **does that tone actually move markets**, and **what does both of those tell us about the September 15-16, 2026 FOMC meeting**. This is a genuine ex-ante forecast, since that meeting's outcome is announced after this project's deadline.
...
- **Forecast:** a from-scratch, text-based rate-decision classifier feeding an ordinal logistic regression. Real market pricing (CME FedWatch, Kalshi, Polymarket), found via live research, is used only as an external sanity check in the write-up, it is never fed into the model or blended into the reported probabilities.

## Key results

- **A real, concrete hawkish pivot:** Powell's Fed cut rates six meetings in a row (mid-2025 to April 2026); Warsh then **held** at his first meeting (June 2026) and **hiked** at his second (July 2026), found by classifying every statement's decision directly from its own text.
- **A confound, caught and correctly handled:** the 1-year Treasury regression showed roughly 30% R-squared across all four tone methods, nearly identical, which was suspicious. Checking the control confirmed the 3-month bill was doing almost all the work (coefficient approx. 0.8-0.9, p<0.0001); net of that, most individual tone coefficients were *not* statistically significant. This is exactly the confound the assignment's control is designed to catch.
- **External validation we didn't expect:** real market pricing in early September 2026 independently priced the meeting as a "coin flip," driven by the same catalyst (Warsh's Aug 28 Jackson Hole speech) our text-only model flagged on its own. This is used only as a sanity-check comment in the report, not as an input to our own number.
- **Final forecast:** P(cut) approx. 0.8%, P(hold) approx. 23.7%, P(hike) approx. 75.5%, this is our own model's raw output, reported as is and not adjusted toward the market's more cautious "coin flip" framing, since the assignment asks for our own forecast. We do flag that the model is likely somewhat overconfident given a 68-meeting training set dominated by decision-momentum, not tone, as an honest caveat rather than as a reason to change the number.
- **A data bug found and fixed post-delivery:** minutes were originally assigned the meeting date as their release date instead of their true ~3-week-later publication date, silently duplicating that day's statement reaction onto the minutes row. Caught by a user question, fixed, and verified to leave the primary (statements-only) results and the forecast completely unchanged, see Step 17 below and `AI_USE.md`.
- **A conceptual bug found and fixed post-delivery:** the market-reaction forecast was plugging July's own tone into a regression *fit using July's own row*, mathematically just returning July's already-known fitted value, not a September forecast. Redesigned collaboratively into a family of models sharing one lagged predictor set; Table 3 itself deliberately left unretrained. See Step 18 below and `AI_USE.md`.

## How this was built, step by step

**1-2. Understanding the assignment and reading the papers.** No code involved here. We verified via web search that Warsh's Fed-chair tenure is a real, recent event (postdates training data), not a hypothetical, and pinned down the exact 2026 timeline before writing anything. Read all three assigned papers in full and found "Parsing the Fed" was structurally a prior version of this exact assignment, letting us reuse its exact formulas rather than inventing our own. Also found the deeper Doh et al. method is structurally inapplicable to recent data, since the Fed's internal "alternative statements" are only declassified after a 5-year lag.

**3. Environment and repo setup.** Checked Python 3.14 had no PyTorch wheel yet before committing to it, used 3.13 instead. `.gitignore` and `.env` (holding the FRED key) were created before `git init`, so a credential could never accidentally get committed.
...
**5. Scraping speeches and testimony.** `scrape_speeches_testimony.py` finds every Fed speech/testimony page by speaker surname in the URL, then filters to each person's *actual tenure as Chair*, so a Powell speech after May 15, 2026 is excluded even though he remained a Governor and kept speaking. **Result: 78 speeches, 26 testimony documents, zero failures.**
...
**11. Master dataset and regressions.** `build_master_dataset.py` merges everything into one table; `run_regressions.py` implements Table 3, regressing each indicator on each method's score(s) jointly, controlling for the 3-month bill. This is where the DGS3MO confound described above was caught.

**12. Figures, tables and rate decisions.** `make_figure1.py` builds the 3-panel tone-over-time chart; `make_table2.py` isolates the Warsh-era releases; `rate_decisions.py` classifies every statement's actual decision (cut/hold/hike) directly from its text via regex, surfacing the hold-to-hike pivot that became the forecast's empirical backbone.

**13. The forecast model.** `forecast.py` fits an ordinal logistic regression for rate-decision probabilities, estimates statement-tone momentum, and converts Table 3's regression coefficients into market-reaction probabilities. A live web search for real market pricing provided the external cross-check described above. *(Post-delivery redesign: the original market-reaction and tone-momentum logic had a real conceptual flaw, see Step 18.)*

**14. Notebook assembly.** `notebooks/analysis.ipynb` was built cell by cell, loading each script's saved output rather than re-running the full ~15-20 minute pipeline inline, then executed top to bottom and verified programmatically to have zero cell errors.
...
**16. Finalizing and shipping.** `AI_USE.md` written with a specific account of what was AI-generated vs. jointly decided. Verified via `git ls-files` that no data or secrets were ever committed.
...
**18. Forecast redesign.** Explaining `market_reaction_forecast()`'s mechanism plainly exposed that it couldn't be forecasting anything: it plugged July's own tone into a regression *fit using July's own row*, so it mathematically just returned July's already-known fitted value. Several designs were discussed and rejected (decision-probability-weighted conditional means, a plain AR(1) per topic, an intermediate version needing an estimated `E[decision_code(September)]`, retraining Table 3 itself with lagged inputs, rejected because that would answer a different, economically dubious question and stop matching what Table 3 is actually required to test) before landing on: one shared **lagged** predictor set (each meeting's own decision plus all four word-list topic scores, one meeting back) training three model types that differ only in their target: an ordinal logit for the decision, four OLS regressions forecasting September's own tone from July's real values, and a logistic regression for "more hawkish than last time." Table 3 itself is never retrained; it just receives a real forecast instead of July's raw numbers. Verified after building: `lag_decision_code` remained the dominant, significant predictor even after adding two more topics (both came back insignificant, an honest finding not a flaw); the new tone-momentum estimate (~10.6%) is independently corroborated by the separate tone-forecast model, which also predicts September below July for the same reason (mean reversion after an already-elevated reading); Table 3's own numbers were confirmed untouched.
...
- **`AI_USE.md`** - how AI assistance was used, specifically, including two mistakes made and fixed after initial delivery

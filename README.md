...
**7. Building the corpus.** `build_corpus.py` merges both scrapes into one table and computes each document's release hour — the detail that later decides which market-close window is the correct one to measure. **Result: `corpus.parquet`, 311 rows, 1.34M words total.**
...
docs/           STORY.md + 16 step-by-step build docs (fuller version of the story above)

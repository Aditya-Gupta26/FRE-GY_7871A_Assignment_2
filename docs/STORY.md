...
| 18 | [Forecast Redesign](steps/18-forecast-redesign.md) | A deeper, conceptual problem found through discussion: the market-reaction forecast was plugging July's tone into a regression fit on July's own row, mathematically returning July's own fitted value, not a forecast. Redesigned collaboratively through several rejected options into one shared lagged-predictor model family; Table 3 itself deliberately left unretrained | `forecast.py` |
...
- **Planning record:** `PLAN.md` (written *before* execution, plus a Section 8 execution log, a Section 9 post-delivery bug-fix plan, and a Section 10 forecast-redesign record, all appended *after*, so you can see intention vs. outcome side by side)

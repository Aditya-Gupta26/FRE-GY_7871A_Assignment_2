...
> **Correction added after initial delivery** (full story in [Step 17](17-post-delivery-bug-fix.md)): this step originally stored the *meeting* date as a minutes document's only date field, which downstream code incorrectly read as its release date — minutes actually publish about three weeks later. `scrape_minutes()` now also computes the true publication date (verified against 5 independently-confirmed real dates), stored separately so the meeting-date identifier used elsewhere is untouched. This paragraph is left here, uncorrected in its original claims, specifically so the mistake is visible in the story rather than quietly edited away.

...
> **Correction added after initial delivery** (full story in [Step 17](17-post-delivery-bug-fix.md)): the *tone-score* columns above (`wl_interest_rate` etc.) were always correct — they come from document text, unaffected by the issue below. But the *market-reaction* columns (`d_10s2s`, `d_1yr`, `d_DXY_pct`, `d_growth_value`) for this table's two minutes rows were originally wrong: they showed the exact same numbers as that day's statement and press-conference rows, because [Step 10](10-event-study.md)'s event study was (incorrectly) anchoring on the meeting date rather than minutes' true ~3-week-later publication date. Table 2 now correctly shows the minutes rows with their own, different market-reaction numbers.
